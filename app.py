...
def get_supabase_client():
    try:
        ...
        raise RuntimeError(
            ...
            "SUPABASE_KEY in Streamlit Cloud > Settings > Secrets."
        ) from exc

    if not str(url).strip() or not str(key).strip():
        raise RuntimeError("SUPABASE_URL or SUPABASE_KEY is empty.")

    return create_client(str(url).strip(), str(key).strip())


def _safe_storage_class(prediction):
    value = str(prediction).strip()
    if not value:
        raise RuntimeError("The model returned an empty disease class.")
...
def save_diagnosis_to_supabase( uploaded_file, crop, prediction, confidence, needs_human_review, ):
    supabase = get_supabase_client()
    storage_path = None

    try:
        now = datetime.now(timezone.utc)
        disease_class = _safe_storage_class(prediction)
        date_folder = now.strftime("%Y/%m/%d")
        unique_id = uuid.uuid4().hex

        original_name = uploaded_file.name or "tomato_leaf.jpg"
        ...
        storage_path = (
            f"{disease_class}/{date_folder}/{unique_id}{extension}"
        )

        image_bytes = uploaded_file.getvalue()
        if not image_bytes:
            raise RuntimeError("Uploaded image contains no data.")
        ...
        raise RuntimeError(
            f"Supabase save failed: {exc}"
        ) from exc


st.markdown(
    f""" <style> {background_css} #MainMenu,footer{{visibility:hidden}} header{{background:transparent!important}} .block-container{{max-width:1180px;padding-top:1rem;padding-bottom:4rem}} .nav{{display:flex;justify-content:space-between;align-items:center;padding:15px 5px 20px;color:white}} .brand{{font-size:28px;font-weight:900}} .brand span,.hero h1 span{{color:#91e66d}} .nav-right{{color:rgba(255,255,255,.65);font-size:12px;letter-spacing:1.5px}} .hero{{text-align:center;color:white;padding:50px 15px 55px}} .badge{{display:inline-block;padding:8px 16px;border-radius:30px;background:rgba(145,230,109,.15);border:1px solid rgba(145,230,109,.35);color:#a8ee89;font-size:13px;font-weight:800;margin-bottom:20px}} .hero h1{{font-size:clamp(42px,7vw,75px);line-height:.98;letter-spacing:-3px;margin:0;font-weight:900}} .hero p{{max-width:700px;margin:24px auto 0;font-size:18px;line-height:1.6;color:rgba(255,255,255,.82)}} .glass{{background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.17);border-radius:25px;padding:28px;backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);box-shadow:0 25px 70px rgba(0,0,0,.22);color:white}} .glass h2,.glass h3{{color:white}} .result{{background:rgba(255,255,255,.97);border-radius:25px;padding:30px;color:#142519;box-shadow:0 25px 70px rgba(0,0,0,.28)}} .label{{color:#65806c;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px}} .diagnosis{{color:#142519;font-size:35px;font-weight:900;margin:6px 0 15px}} .confidence{{color:#18351d;font-size:40px;font-weight:900}} .warning-box{{background:#fff3d6;border-left:5px solid #d89b19;padding:18px;border-radius:12px;color:#654600;margin-top:18px}} .success-box{{background:#e7f8e2;border-left:5px solid #4b9b3f;padding:18px;border-radius:12px;color:#245d20;margin-top:18px}} [data-testid="stFileUploader"]{{background:rgba(255,255,255,.07);border:2px dashed rgba(142,226,107,.55);border-radius:20px;padding:10px}} .stButton>button{{width:100%;border-radius:14px;border:none;background:#83d95f;color:#102411;font-weight:900;padding:14px 20px;font-size:16px}} .stButton>button:hover{{background:#a0ed7e}} .footer{{text-align:center;color:rgba(255,255,255,.60);padding:45px 10px 15px;font-size:13px}} @media(max-width:768px){{.nav-right{{display:none}}.hero{{padding:35px 10px 45px}}.hero h1{{letter-spacing:-2px}}.result,.glass{{padding:21px}}.diagnosis{{font-size:28px}}}} </style> """,
    unsafe_allow_html=True,
)

st.html(
    """ <div class="nav"> <div class="brand">GAIA<span>🍅</span></div> <div class="nav-right">TOMATO HEALTH INTELLIGENCE</div> </div> <div class="hero"> <div class="badge">✦ AI-POWERED CROP HEALTH</div> <h1>Know your crop.<br><span>Grow with confidence.</span></h1> <p>Upload a tomato leaf image and GAIA will screen it for 11 tomato health and disease conditions using a Vision Transformer.</p> </div> """
)
...
if not isinstance(CLASS_NAMES, list) or not CLASS_NAMES:
    st.error("GAIA model configuration contains no classes.")
    st.stop()
...
st.markdown(
    """ <div class="glass"> <h2>🔬 Analyze a tomato leaf</h2> <p>Upload a clear JPG, JPEG or PNG photograph. Good lighting, focus and a visible leaf generally provide better screening conditions.</p> </div> """,
    unsafe_allow_html=True,
)
...
if uploaded is not None:
    ...
    with right:
        if not analyze:
            st.html(
                """ <div class="result"> <div class="label">READY</div> <div class="diagnosis">Ready to analyze</div> <p>Click <b>Analyze with GAIA</b> to run the trained Vision Transformer.</p> </div> """
)
        else:
            try:
                ...
                save_result = save_diagnosis_to_supabase(
                ...
                st.html(
                    f""" <div class="result"> <div class="label">GAIA DETECTION</div> <div class="diagnosis">{name}</div> <div class="label">CONFIDENCE</div> <div class="confidence">{confidence_pct:.2f}%</div> {box} <br> <b>Status:</b> {status}<br> <b>Severity:</b> {info["severity"]} </div> """
)
                ...
                st.success(
                    f"Saved to Supabase: `{save_result['storage_path']}`"
                )
                ...
                st.html(
                    f""" <div class="result"> <div class="label">WHAT GAIA DETECTED</div> <h2>{name}</h2> <p>{info["description"]}</p> <hr> <div class="label">RECOMMENDED NEXT STEP</div> <p>{info["action"]}</p> </div> """
)
    ...
    st.html(
        """ <div class="glass" style="text-align:center;margin-top:35px;"> <div style="font-size:52px;">🍃</div> <h2>Your crop health starts here</h2> <p>Upload a tomato leaf photograph above to begin AI-assisted disease screening.</p> </div> """
)

st.html(
    """ <div class="footer"> <strong>GAIA Tomato AI</strong><br><br> AI-assisted tomato crop health screening.<br> Results are intended to support agricultural decision-making and should not replace assessment by a qualified plant-health professional. </div> """
)
